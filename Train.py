...
def train(n_epochs, optimizer, model, loss_fn, train_loader, scheduler, device):
    ...
    for epoch in range(1, n_epochs+1):
        ...
        for imgs, labels in train_loader:  # for each batch of 2048 images
            # Create an empty tesnor that's 2048*28*28
            imgs2 = torch.zeros(imgs.size()[0], 784)
...
train_dataloader = DataLoader(
    train_set, batch_size=int(args.BatchSize), shuffle=True)
scheduler = lr_scheduler.LinearLR(
    optim, start_factor=1.0, end_factor=0.3, total_iters=10)
device = (
    "cuda"
...
loss_data = train(n_epochs=50, optimizer=optim, model=model, loss_fn=loss,
                  train_loader=train_dataloader, scheduler=scheduler, device=device)
